...
@everywhere function integralimage(A::Matrix{T}) where {T <: Number}
  n,m = size(A)
...
@everywhere function integralimage(A::Matrix{Union{T, Missing}}) where {T <: Number}
  integralimage(replace(A, missing => zero(T)))
end
