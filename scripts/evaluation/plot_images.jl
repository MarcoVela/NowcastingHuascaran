...
tx = copy(selectdim(tx, 4, 1:8))
ty = copy(selectdim(ty, 4, 1:8))
W,H = size(tx)[1:2]
ty = reshape(ty, W, H, :)
rtx = reshape(tx, W, H, :)
...
preds = [reshape(model(tx), W, H, :) for model in models]

labels = [["model_$i" for i in 1:length(models)]..., "Truth"]
samples = [preds..., ty]

plot_many(samples, labels; layout=(1, length(labels)), size=(900, 400), fps=1)
