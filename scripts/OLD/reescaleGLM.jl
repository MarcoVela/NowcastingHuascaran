...
@everywhere using NCDatasets, DataStructures, ProgressMeter, Dates, Base.Threads, Statistics
...
@everywhere function sumgrid2(grid, k)
  n,m = size(grid)
  sn, sm = n÷k, m÷k
  dropdims(sum(reshape(@view(grid[begin:sn*k, begin:sm*k]), k, sn, k, sm), dims=(1,3)); dims=(1,3))
end
...
@everywhere function reescale(ds::OwnDataset)
  k = ds.k
  n = (length(ds.latitude) ÷ k) * k
  n2 = (length(ds.longitude) ÷ k) * k
  OwnDataset(
    sumgrid2(ds.density, k),
    mean(reshape(ds.latitude[begin:n], k, :), dims=1)[:],
    mean(reshape(ds.longitude[begin:n2], k, :), dims=1)[:],
    ds.time,
    ds.level,
    ds.time_bounds,
    ds.file_names_used,
...
function main()
  ...
  local_paths_chan = Channel{Tuple{String,String}}(length(paths))

  chan_limit = min(length(paths) ÷ 7, 3_000)
  paths_chan = RemoteChannel(() -> local_paths_chan)
  reescaled_chan = RemoteChannel(() -> Channel{OwnDataset}(chan_limit))
  own_dataset_chan = RemoteChannel(() -> Channel{OwnDataset}(chan_limit))
