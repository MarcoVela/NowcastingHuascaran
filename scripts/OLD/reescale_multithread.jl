...
using NCDatasets, DataStructures, ProgressMeter
...
function recreatedataset(dataset, newpath, k)
...
function recreate(path, foldername, newfoldername, k)
  ...
  pbar = Progress(length(files); dt=1, desc="Recreando archivos...")

  println("Working with $(nthreads())")
	@threads for (file, newfile) in collect(zip(files, newfiles))
