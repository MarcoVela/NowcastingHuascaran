...
function (m::Conv2dLSTMCell)((h, c), x::AbstractArray{T, 3}) where {T}
  gates = m.Wxh(x)::AbstractArray{T, 3} .+ m.Whh(h)::AbstractArray{T, 3}
...
function (m::Conv2dLSTMCell)((h, c), x::AbstractArray{T, 4}) where {T}
  gates = m.Wxh(x)::AbstractArray{T, 4} .+ m.Whh(h)::AbstractArray{T, 4}
  wi,he,ch,N = size(gates)
  origch = ch ÷ 4
  input_gate, forget_gate, cell_gate, output_gate = map(chunk -> reshape(chunk, wi, he, origch, N), Flux.chunk(gates, 4))
